/** @vitest-environment jsdom */
import { render, screen } from "@testing-library/react";
import userEvent from "@testing-library/user-event";
import { describe, expect, it } from "vitest";
import { createRuntime } from "../runtime";
import { App } from "./App";

describe("Phase 3 researcher demo", () => {
  it("time-travel occasion reminder leads to a purchase decision", async () => {
    const user = userEvent.setup();
    render(<App runtime={createRuntime()} />);

    await user.click(screen.getByRole("button", { name: /Pretend trip is in 3 days/i }));
    await user.click(screen.getByRole("button", { name: /^Wishlist$/i }));
    expect(screen.getByText("Trip in 3 days")).toBeTruthy();

    await user.click(screen.getByRole("button", { name: /Notifications/i }));
    await user.click(screen.getByRole("button", { name: /Trip in 3 days/i }));
    await user.click(screen.getByRole("button", { name: "Add to Bag" }));
    await user.click(screen.getByRole("button", { name: /Continue to checkout/i }));
    await user.click(screen.getByRole("button", { name: /Place order/i }));
    expect(screen.getByText("Order successful")).toBeTruthy();
  });
});
